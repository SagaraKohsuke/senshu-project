...
function getInitialData() {
  ...
  if (!allData.users || !allData.b_menus || !allData.d_menus) {
    Logger.log('初期データ取得エラー: users, b_menus, d_menus シートのいずれかが存在しません');
    throw new Error('初期データ取得エラー: users, b_menus, d_menus シートのいずれかが存在しません');
  }
...
function getCalendarData(userId) {
  const allData = getAllSheetsData();
  if (!allData.b_calendar_202503 || !allData.d_calendar_202503 || !allData.b_reservation_202503 || !allData.d_reservation_202503) {
    Logger.log('カレンダーデータ取得エラー: カレンダーまたは予約シートが存在しません');
    throw new Error('カレンダーデータ取得エラー: カレンダーまたは予約シートが存在しません');
  }
...
function updateReservation(userId, calendarId, mealType, isReserved) {  
  ...
  if (idColumnIndex === 0 || calendarIdColumnIndex === 0 || isReservedColumnIndex === 0) {
    Logger.log('必要な列が見つかりません');
    return { success: false, error: 'Required column not found in sheet: ' + sheetName };
  }
  ...
  for (var i = 1; i < lastRow; i++) {
    if (values[i][calendarIdColumnIndex - 1] === calendarId && String(values[i][1]) === String(userId)) { // user_id は2列目と仮定
      sheet.getRange(i + 1, isReservedColumnIndex).setValue(isReserved ? 'TRUE' : 'FALSE');
      return { success: true };
    }
  }

  if (mealType === 'breakfast') {
    sheet.appendRow([Utilities.getUuid(), userId, calendarId, isReserved ? 'TRUE' : 'FALSE']);
  } else if (mealType === 'dinner') {
    sheet.appendRow([Utilities.getUuid(), userId, calendarId, isReserved ? 'TRUE' : 'FALSE']);
  }
